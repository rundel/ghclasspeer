...
peer_file_add_rev = function(org, roster, local_path, prefix = "", suffix = "",
  ...
  ghclass::arg_is_lgl(overwrite, verbose)
  ...
  if (any(!file_status))
    ghclass::cli_stop("Unable to locate the following file(s): {.val {local_path[!file_status]}}")
  ...
  purrr::walk(
    rev,
    function(rev) {
      repo_files = ghclass::repo_files(rev, branch = branch)
