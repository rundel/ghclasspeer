...
peer_roster_create = function(n_rev, user, seed = NULL, write_csv = TRUE, dir = NULL) {
  ghclass::arg_is_chr(user)
  ghclass::arg_is_chr(dir, allow_null = TRUE)
  ghclass::arg_is_pos_int(n_rev)
  ghclass::arg_is_pos_int_scalar(seed, allow_null = TRUE) #fails now

  if (!(length(user) > 1)) {
    ghclass::cli_stop("{.field user} must contain more than one user name.")
  }

  if (!(n_rev < length(user))) {
    ghclass::cli_stop("{.field n_rev} must be smaller than the number of users in {.field user}.")
  }

  if (write_csv) {
    if (is.null(dir)) {
      ghclass::cli_stop("No directory specified in {.field dir}.")
    } else if (!dir.exists(dir)) {
      ghclass::cli_stop("Directory {.val {dir}} does not exist.")
    }
  }

  if (is.null(seed)) {
    seed = sample.int(1e+06, 1L)
    ghclass::cli_warn("No seed was specified. Using randomly sampled seed {.val {seed}}")
  }
