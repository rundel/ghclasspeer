...
local_path_content_grab = function(local_path = NULL, check_rmd = TRUE) {
  ghclass::arg_is_chr(local_path, allow_null = TRUE)
  ghclass::arg_is_lgl_scalar(check_rmd)
  ...
  purrr::map(
    ...
    function(local_path) {
      if (!is.null(local_path)) {
        ...
        if (!file_status)
          ghclass::cli_stop("Unable to locate the following file: {.file {local_path}}")
        ...
        if (check_rmd & (tolower(fs::path_ext(local_path)) == "rmd")) {
          ...
          ghclass::cli_stop("{.field local_path} must be an {.file .Rmd} file.")
        }
      }
    }
  )
...
repo_files_select = function(repo, repo_files = NULL, exclude_pattern, branch) {
  ghclass::arg_is_chr_scalar(repo, branch)
  ghclass::arg_is_chr(exclude_pattern, allow_null = T)

  if (is.null(repo_files)) {
    repo_files = ghclass::repo_files(repo = repo, branch = branch)
  }
...
format_commit_output = function(res = NULL, target_files = NULL, target_repo, target_path,
  ...
  ghclass::arg_is_chr_scalar(target_repo, target_path, target_folder, category)
  ghclass::arg_is_lgl_scalar(changed, allow_na = TRUE)
  ...
  if (is.null(res) & !is.null(target_files)) {
    ...
    added = FALSE
    commit_sha = NA
  } else if (!is.null(res) & ghclass::succeeded(res)) {
...
peer_add_content = function(target_repo, target_folder, target_files, content,
  ...
  ghclass::arg_is_chr(target_repo)
  ghclass::arg_is_chr_scalar(category, target_folder)
  ghclass::arg_is_chr_scalar(message, branch, allow_null = TRUE)
  ghclass::arg_is_lgl(overwrite)
...
repo_path_content_grab = function(repo, path, repo_files = NULL, branch) {
  ...
  purrr::map(
    ...
    function(path) {
      ...
      if (path_exists) {
        res = purrr::safely(ghclass::repo_get_file)(repo = repo,
                                           path = path,
                                           branch = branch)

        if (ghclass::succeeded(res))
