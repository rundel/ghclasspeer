...
peer_issue_label_apply = function(org, repo = NULL, filter = NULL,
  ...
  ghclass::arg_is_chr_scalar(org)
  ghclass::arg_is_chr_scalar(filter, allow_null = TRUE)
  ghclass::arg_is_chr(repo, allow_null = TRUE)
  ghclass::arg_is_lgl(exclude, verbose, show_label_result)
...
issue_txt_complete_review = function(sub, aut, url_start) {
  ...
  if (nrow(tmp) > 0) {
    ghclass::arg_is_chr_scalar(tmp[['path']])
