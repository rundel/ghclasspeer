...
peer_score_rating = function(org, roster, form_rating, double_blind = TRUE,
  ...
  # Checks
  ghclass::arg_is_chr_scalar(org, prefix, suffix, form_rating)
  ghclass::arg_is_lgl(double_blind, write_csv)

  # Check that feedback form is .Rmd
  if (!grepl("\\.[rR]md$", form_rating)) {
    ghclass::cli_stop("{.field form_rating} must be an {.file .Rmd} file.")
  }
  ...
  out_temp = purrr::map_dfr(
    ...
    function(x) {
      ...
      feedback = purrr::safely(ghclass::repo_get_file)(repo = repo, path = ghpath)

      if (ghclass::succeeded(feedback)) {
